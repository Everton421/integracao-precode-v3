...
class ProdutoGradeMapper
{
    ...
    private function buscarDadosPai($codigoGrade)
    {
        $sql = "
            SELECT
              p.GRADE, p.CODIGO, p.OUTRO_COD, p.DATA_RECAD, 
              p.SKU_MKTPLACE, p.DESCR_CURTA_MKTPLACE, p.DESCR_LONGA_MKTPLACE,
              g.DESCRICAO, g.APLICACAO, P.GARANTIA,
              p.COMPRIMENTO, p.LARGURA, p.ALTURA, p.PESO,
              p.ORIGEM, p.CATEGORIA_MKTPLACE, p.INTERM_CATEGORIA_MKTPLACE,
              p.FINALCATEGORIA_MKTPLACE, p.MODELO_MKTPLACE, p.NUM_FABRICANTE,
            ...
            AND tp.tabela = {$this->tabelaPreco} AND g.CODIGO = '$codigoGrade'
            LIMIT 1
        ";
